...
module.exports = function initPlugin(sails, cb) {
  ...
  sails.on('we:model:contact:requested', function(contact, actor, contactUser) {
    WN.notify('contact', 'requested', contact, actor, contactUser);
  });
  // create one notification on contact accept
  sails.on('we:model:contact:accepted', function(contact, actor, contactUser) {
    WN.notify('contact', 'accepted', contact, actor, contactUser);
  });

  // // create one notification on contact accept
  // sails.on('we:model:mention:afterCreate', function(mention) {
  //   sails.log.warn('RODO1!', mention)
